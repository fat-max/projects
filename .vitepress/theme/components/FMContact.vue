<template>
  <div class="p-6 max-w-lg">
    ...
    <form class="mt-8 space-y-5" method="POST" action="https://submit-form.com/v55s8XsXC">
      <input type="hidden" name="_redirect" value="https://fatmax.se/contact/thanks" />
      <div>
      ...
      <div>
        ...
        <textarea placeholder="Enter Message" rows="6" id="message" name="message" required=""
          class="w-full px-2 bg-gray-100 border border-gray-200 text-slate-800 focus:border-slate-900 text-sm pt-3 outline-0 transition-all"></textarea>
      </div>
      <div class="g-recaptcha" data-theme="dark" data-sitekey="6LcrQHQsAAAAANmCwLGAKPjTQkrqUYecH65tk04K"></div>
      <button type="submit"
        class="text-white bg-orange-900 font-medium hover:bg-orange-800 tracking-wide text-sm px-2 py-1.5 w-full border-0 outline-0 cursor-pointer">Send
        message</button>
    </form>
  </div>
</template>